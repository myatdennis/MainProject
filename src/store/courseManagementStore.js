...
class CourseManagementStore {
    ...
    addChapterToCourse(courseId, chapterData) {
    ...
    addLessonToChapter(courseId, chapterId, lessonData) {
        const course = this.courses.get(courseId);
        if (!course)
            return null;
    ...
    // Progress Tracking
    enrollLearner(learnerId, courseId) {
    ...
    updateProgress(learnerId, courseId, updates) {
        const progressId = `${learnerId}-${courseId}`;
    ...
    updateLessonProgress(learnerId, courseId, lessonId, lessonProgress) {
        const progressId = `${learnerId}-${courseId}`;
        const progress = this.learnerProgress.get(progressId);
        if (!progress)
            return false;
        ...
        for (const chapterProgress of progress.chapterProgress) {
            const lessonProgressItem = chapterProgress.lessonProgress.find((lp) => lp.lessonId === lessonId);
            if (lessonProgressItem) {
                ...
                const completedLessons = chapterProgress.lessonProgress.filter((lp) => lp.status === 'completed').length;
                chapterProgress.progress = (completedLessons / totalLessons) * 100;
                // Recalculate overall progress
                const totalChapters = progress.chapterProgress.length;
                const overallProgress = progress.chapterProgress.reduce((sum, cp) => sum + cp.progress, 0) / totalChapters;
    ...
    // Bookmarks and Notes
    addBookmark(learnerId, courseId, bookmarkData) {
        const progressId = `${learnerId}-${courseId}`;
        const progress = this.learnerProgress.get(progressId);
        if (!progress)
            return null;
    ...
    addNote(learnerId, courseId, noteData) {
        const progressId = `${learnerId}-${courseId}`;
        const progress = this.learnerProgress.get(progressId);
        if (!progress)
            return null;
    ...
    markLessonComplete(learnerId, courseId, lessonId) {
        const progressId = `${learnerId}-${courseId}`;
        const progress = this.learnerProgress.get(progressId);
        if (!progress)
            return;
    ...
    getUserNotes(userId, courseId) {
        const progressId = `${userId}-${courseId}`;
        const progress = this.learnerProgress.get(progressId);
        return progress?.notes || [];
    }
    getUserBookmarks(userId, courseId) {
        const progressId = `${userId}-${courseId}`;
        const progress = this.learnerProgress.get(progressId);
        return progress?.bookmarks || [];
    }
    // Course Recommendations
    getRecommendationsForLearner(learnerId, limit = 5) {
    ...
    searchCourses(query, filters) {
        ...
        // Text search
        if (query) {
            results = results.filter(course => course.title.toLowerCase().includes(query.toLowerCase()) ||
    ...
    getLearnerProgress(learnerId, courseId) {
        const progressId = `${learnerId}-${courseId}`;
