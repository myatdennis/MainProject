...
beforeEach(() => {
  ...
  vi.resetModules();
  vi.clearAllMocks();
  adminCoursesMock.getAllCoursesFromDatabase.mockReset();
  adminCoursesMock.syncCourseToDatabase.mockReset();
  adminCoursesMock.deleteCourseFromDatabase.mockReset();
  clientCoursesMock.fetchPublishedCourses.mockReset();
  clientCoursesMock.fetchCourse.mockReset();
  assignmentStorageMock.getAssignmentsForUser.mockReset();
  assignmentStorageMock.getAssignmentsForUserWithOutcome.mockReset();
  courseProgressMock.loadStoredCourseProgress.mockReset();
  courseAvailabilityMock.hasStoredProgressHistory.mockReset();
  courseDraftsMock.saveDraftSnapshot.mockReset();
  courseDraftsMock.markDraftSynced.mockReset();
  courseDraftsMock.deleteDraftSnapshot.mockReset();
  secureStorageMock.getUserSession.mockReset();
  secureStorageMock.getActiveOrgPreference.mockReset();
  secureStorageMock.getAccessToken.mockReset();
  secureStorageMock.secureGet.mockReset();
  secureStorageMock.secureSet.mockReset();
  secureStorageMock.secureRemove.mockReset();
  supabaseClientMock.auth.getSession.mockReset();
  supabaseClientMock.auth.refreshSession.mockReset();
  supabaseClientMock.auth.onAuthStateChange.mockReset();
  apiAccessTokenMock.mockReset();
  runtimeStatusMock.getRuntimeStatus.mockReset();
  runtimeStatusMock.refreshRuntimeStatus.mockReset();
