...
const _loadCoursesFromLocalStorage = () => ({});
const _saveCoursesToLocalStorage = (_courses) => {
...
let courses = _loadCoursesFromLocalStorage();
...
export const courseStore = {
            ...
            if (dbCourses.length > 0) {
                ...
                for (const course of Object.values(defaultCourses)) {
                    try {
                        ...
                        if (persisted) {
                            const normalized = normalizeCourse(persisted);
                        ...
                        if (error instanceof CourseValidationError) {
                            console.warn(`Default course "${course.title}" failed validation during seed: ${error.issues.join(' | ')}`);
                        }
                        else {
            ...
            for (const course of Object.values(defaultCourses)) {
                try {
                    ...
                    if (seedError instanceof CourseValidationError) {
                        console.warn(`Skipped syncing default course "${course.title}" due to validation issues: ${seedError.issues.join(' | ')}`);
                    }
                }
            }
        }
    ...
    saveCourse: (course, options = {}) => {
        ...
        if (!options.skipRemoteSync &&
            ...
            CourseService.syncCourseToDatabase(course)
                ...
                .catch(error => {
                if (error instanceof CourseValidationError) {
                    console.warn(`Skipped remote sync for course "${course.title}" due to validation errors: ${error.issues.join(' | ')}`);
                    return;
                }
    ...
    createCourse: (courseData) => {
        const generatedId = courseData.id ||
        ...
        CourseService.syncCourseToDatabase(newCourse)
            ...
            .catch((error) => {
            if (error instanceof CourseValidationError) {
                console.warn(`New course "${newCourse.title}" failed validation when syncing: ${error.issues.join(' | ')}`);
                return;
            }
...
export const countTotalLessons = (modules) => {
