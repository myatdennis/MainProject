...
// Minimal Org/Member shapes for DAL consumers; keep broad to avoid tight coupling
export type Org = Record<string, any>;
export type OrgMember = Record<string, any>;
...
const mapOrgRecord = (record: any): Org => ({
  ...
  contactPhone: record.contact_phone ?? record.contactPhone ?? undefined,
  ...
  totalLearners: Number(record.total_learners ?? record.totalLearners ?? 0),
  activeLearners: Number(record.active_learners ?? record.activeLearners ?? 0),
  completionRate: Number(record.completion_rate ?? record.completionRate ?? 0),
  cohorts: record.cohorts ?? [],
  lastActivity: record.last_activity ?? record.lastActivity ?? undefined,
...
export const listOrgs = async (): Promise<Org[]> => {
  const json = await request<{ data: any[] }>('/api/admin/organizations');
  if ((json.data?.length ?? 0) === 0) {
    for (const org of seedOrgs()) {
      await createOrg(org);
    }
    const refreshed = await request<{ data: any[] }>('/api/admin/organizations');
...
export const updateOrg = async (id: string, patch: Partial<Org>): Promise<Org> => {
  const json = await request<{ data: any }>(`/api/admin/organizations/${id}`, {
    method: 'PUT',
...
export const deleteOrg = async (id: string): Promise<void> => {
  await request(`/api/admin/organizations/${id}`, { method: 'DELETE' });
};

export const listOrgMembers = async (orgId: string): Promise<OrgMember[]> => {
  const json = await request<{ data: any[] }>(`/api/admin/organizations/${orgId}/members`);
...
export const removeOrgMember = async (orgId: string, membershipId: string): Promise<void> => {
  await request(`/api/admin/organizations/${orgId}/members/${membershipId}`, {
    method: 'DELETE',
  });
};
...
export const getOrgStats = async (id: string): Promise<any> => {
  ...
  return {
    ...
    performance: {
      coursesCompleted: Object.values(org.modules || {}).reduce((sum: number, val: any) => sum + Math.floor(Number(val) * 0.8), 0),
      certificatesIssued: Math.floor(Number(org.totalLearners) * (Number(org.completionRate) / 100)),
    ...
    trends: {
      userGrowth: Array.from({ length: 12 }, (_, i) => ({
        ...
        users: Math.floor(Math.random() * 10) + Number(org.totalLearners) * 0.8,
      })),
