...
export const listOrgs = async () => {
    const json = await request('/api/admin/organizations');
    if ((json.data?.length ?? 0) === 0) {
        for (const org of seedOrgs()) {
            await createOrg(org);
        }
        const refreshed = await request('/api/admin/organizations');
...
export const updateOrg = async (id, patch) => {
    const json = await request(`/api/admin/organizations/${id}`, {
        method: 'PUT',
...
export const listOrgMembers = async (orgId) => {
    const json = await request(`/api/admin/organizations/${orgId}/members`);
...
export const getOrgStats = async (id) => {
    ...
    return {
        ...
        performance: {
            coursesCompleted: Object.values(org.modules || {}).reduce((sum, val) => sum + Math.floor(Number(val) * 0.8), 0),
