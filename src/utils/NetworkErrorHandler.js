...
export class NetworkErrorHandler {
    static async handleApiCall(apiCall, options = {}) {
        const { retryConfig = {}, errorMessage = 'An unexpected error occurred', showErrorToast = true, logErrors = true } = options;
    ...
    static handleFinalError(error, message, showToast) {
    ...
    static logErrorToService(error, message) {
        const errorReport = {
            message,
    ...
    static async withLoadingState(apiCall, setLoading, options = {}) {
        setLoading(true);
        try {
...
NetworkErrorHandler.defaultRetryConfig = {
    maxAttempts: 3,
    delay: 1000,
    backoffMultiplier: 2
};
