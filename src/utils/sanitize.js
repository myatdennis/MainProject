...
export function sanitizeHTML(dirty, options = {}) {
    ...
    ], allowedAttributes = ['href', 'target', 'rel', 'src', 'alt', 'title', 'class'], allowDataAttributes = false, } = options;
 ...
 */
export function sanitizeBasicHTML(dirty) {
    return sanitizeHTML(dirty, {
 ...
 */
export function createRichTextMarkup(html) {
    return {
        dangerouslySetInnerHTML: {
