...
const mapSupabaseAssignment = (row) => ({
    id: row.id,
...
const persistLocalAssignments = (records) => {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(records));
};
const emitLocalEvent = (type, assignment) => {
    if (SUPABASE_READY)
        return;
...
const withSupabaseFallback = async (supabaseFn, localFn) => {
    if (!SUPABASE_READY) {
...
const buildLocalAssignment = (courseId, userId, overrides = {}) => {
...
export const addAssignments = async (courseId, userIds, options = {}) => {
    ...
    return withSupabaseFallback(async () => {
        ...
        normalizedIds.forEach((userId) => {
            const existingIndex = existing.findIndex((record) => record.courseId === courseId && record.userId === userId);
    ...
    return withSupabaseFallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                return rows.map((row) => mapSupabaseAssignment({
                    id: row.id,
...
export const getAssignment = async (courseId, userId) => {
    const normalized = userId.toLowerCase();
    return withSupabaseFallback(async () => {
        const supabase = await getSupabase();
        if (!supabase)
        ...
        return record ? mapSupabaseAssignment(record) : undefined;
    }, () => loadLocalAssignments().find((record) => record.courseId === courseId && record.userId === normalized));
};
export const updateAssignmentProgress = async (courseId, userId, progress) => {
    const normalized = userId.toLowerCase();
    const now = new Date().toISOString();
    const clampedProgress = Math.min(Math.max(progress, 0), 100);
    const status = clampedProgress >= 100 ? 'completed' : clampedProgress > 0 ? 'in-progress' : 'assigned';
    return withSupabaseFallback(async () => {
        const supabase = await getSupabase();
        if (!supabase)
            throw new Error('Supabase unavailable');
        ...
        const record = Array.isArray(data) ? data[0] : data;
        const assignment = record ? mapSupabaseAssignment(record) : undefined;
        return assignment;
    }, () => {
        const assignments = loadLocalAssignments();
        const index = assignments.findIndex((record) => record.courseId === courseId && record.userId === normalized);
        if (index === -1)
            return undefined;
        const updated = {
            ...assignments[index],
...
export const markAssignmentComplete = async (courseId, userId) => updateAssignmentProgress(courseId, userId, 100);
