...
const compareLessons = (prev, next) => {
    if (!prev || !next)
        return true;
    ...
    if ((prev.type || 'text') !== (next.type || 'text'))
        return true;
    if ((prev.order ?? 0) !== (next.order ?? 0))
        return true;
    if ((prev.estimatedDuration ?? 0) !== (next.estimatedDuration ?? 0))
        return true;
    if (normalizeContent(prev.content) !== normalizeContent(next.content))
        return true;
    if ((prev.description ?? '') !== (next.description ?? ''))
        return true;
    return false;
};
const compareModules = (prev, next) => {
    if (!prev || !next)
        return true;
    if (prev.title !== next.title)
        return true;
    ...
    if (prevLessons.length !== nextLessons.length)
        return true;
    ...
    for (const lesson of nextLessons) {
        ...
        if (!existing)
            return true;
        if (compareLessons(existing, lesson))
            return true;
    }
    return false;
};
export const computeCourseDiff = (previous, next) => {
    if (!previous) {
