...
import { MessageSquare, X, Send, Bot, User, Minimize2, Maximize2, RotateCcw, Lightbulb } from 'lucide-react';
...
const AIBot = () => {
    ...
    const generateBotResponse = (userMessage) => {
        ...
        if (isAdminRoute) {
            ...
            if (lowerMessage.includes('survey') || lowerMessage.includes('climate') || lowerMessage.includes('inclusion') || lowerMessage.includes('equity')) {
                ...
                setMessages(prev => [...prev, { id: placeholderId, type: 'bot', content: 'Fetching analytics...', timestamp: new Date() }].slice(-50));
                (async () => {
                    try {
    ...
    if (!isOpen) {
        return (_jsxs("div", { className: "fixed bottom-6 right-6 z-50 flex flex-col items-end space-y-2", children: [_jsx("button", { onClick: () => setShowTips(s => !s), className: "bg-yellow-400 text-white p-3 rounded-full shadow-lg", title: "Tips & Shortcuts", children: _jsx(Lightbulb, { className: "h-6 w-6" }) }), _jsx("button", { onClick: () => setIsOpen(true), className: "bg-gradient-to-r from-orange-400 to-red-500 text-white p-4 rounded-full shadow-lg hover:from-orange-500 hover:to-red-600 transition-all duration-200 transform hover:scale-110", title: "Open AI Assistant", children: _jsx(MessageSquare, { className: "h-6 w-6" }) }), showTips && (_jsxs("div", { className: "mt-2 w-64 bg-white rounded-lg shadow-lg p-3 border border-gray-200 text-sm text-gray-700", children: [_jsx("div", { className: "font-semibold mb-1", children: "AI Tips" }), _jsxs("ul", { className: "list-disc pl-5 space-y-1", children: [_jsx("li", { children: "Ask for completion rates by org or course." }), _jsx("li", { children: "Use \"Show me users at risk\" to get intervention lists." }), _jsx("li", { children: "Say \"export report\" to prepare summaries." })] })] }))] }));
    }
    return (_jsxs("div", { className: `fixed bottom-6 right-6 bg-white rounded-2xl shadow-2xl border border-gray-200 z-50 transition-all duration-300 ${isMinimized ? 'w-80 h-16' : 'w-96 h-[600px]'}`, children: [_jsxs("div", { className: "flex items-center justify-between p-4 border-b border-gray-200 bg-gradient-to-r from-orange-50 to-red-50 rounded-t-2xl", children: [_jsxs("div", { className: "flex items-center space-x-3", children: [_jsx("div", { className: "bg-gradient-to-r from-orange-400 to-red-500 p-2 rounded-full", children: _jsx(Bot, { className: "h-5 w-5 text-white" }) }), _jsxs("div", { children: [_jsx("h3", { className: "font-bold text-gray-900", children: "AI Assistant" }), _jsx("p", { className: "text-xs text-gray-600", children: isAdminRoute ? 'Admin Support' : isLMSRoute ? 'Learning Coach' : 'The Huddle Co.' })] })] }), _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("button", { onClick: () => setIsMinimized(!isMinimized), className: "p-1 text-gray-400 hover:text-gray-600 rounded", children: isMinimized ? _jsx(Maximize2, { className: "h-4 w-4" }) : _jsx(Minimize2, { className: "h-4 w-4" }) }), _jsx("button", { onClick: clearChat, className: "p-1 text-gray-400 hover:text-gray-600 rounded", children: _jsx(RotateCcw, { className: "h-4 w-4" }) }), _jsx("button", { onClick: () => setIsOpen(false), className: "p-1 text-gray-400 hover:text-gray-600 rounded", children: _jsx(X, { className: "h-4 w-4" }) })] })] }), !isMinimized && (_jsxs(_Fragment, { children: [_jsxs("div", { className: "flex-1 overflow-y-auto p-4 space-y-4 h-96", children: [messages.map((msg) => (_jsx("div", { className: `flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`, children: _jsxs("div", { className: `flex items-start space-x-2 max-w-[80%] ${msg.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`, children: [_jsx("div", { className: `p-2 rounded-full ${msg.type === 'user' ? 'bg-orange-500' : 'bg-gray-100'}`, children: msg.type === 'user' ? (_jsx(User, { className: "h-4 w-4 text-white" })) : (_jsx(Bot, { className: "h-4 w-4 text-gray-600" })) }), _jsxs("div", { children: [_jsx("div", { className: `p-3 rounded-2xl ${msg.type === 'user'
                                                        ? 'bg-gradient-to-r from-orange-400 to-red-500 text-white'
                                                        : 'bg-gray-100 text-gray-900'}`, children: _jsx("p", { className: "text-sm", children: msg.content }) }), msg.suggestions && msg.suggestions.length > 0 && (_jsx("div", { className: "mt-2 space-y-1", children: msg.suggestions.map((suggestion, index) => (_jsx("button", { onClick: () => handleSuggestionClick(suggestion), className: "block w-full text-left text-xs text-blue-600 hover:text-blue-700 bg-blue-50 hover:bg-blue-100 px-2 py-1 rounded-lg transition-colors duration-200", children: suggestion }, index))) })), _jsx("p", { className: "text-xs text-gray-500 mt-1", children: msg.timestamp.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) })] })] }) }, msg.id))), isTyping && (_jsx("div", { className: "flex justify-start", children: _jsxs("div", { className: "flex items-start space-x-2", children: [_jsx("div", { className: "p-2 rounded-full bg-gray-100", children: _jsx(Bot, { className: "h-4 w-4 text-gray-600" }) }), _jsx("div", { className: "bg-gray-100 p-3 rounded-2xl", children: _jsxs("div", { className: "flex space-x-1", children: [_jsx("div", { className: "w-2 h-2 bg-gray-400 rounded-full animate-bounce" }), _jsx("div", { className: "w-2 h-2 bg-gray-400 rounded-full animate-bounce", style: { animationDelay: '0.1s' } }), _jsx("div", { className: "w-2 h-2 bg-gray-400 rounded-full animate-bounce", style: { animationDelay: '0.2s' } })] }) })] }) })), _jsx("div", { ref: messagesEndRef })] }), _jsx("div", { className: "p-4 border-t border-gray-200", children: _jsxs("div", { className: "flex items-center space-x-2", children: [_jsx("input", { type: "text", value: message, onChange: (e) => setMessage(e.target.value), onKeyPress: handleKeyPress, placeholder: "Ask me anything...", className: "flex-1 px-3 py-2 border border-gray-300 rounded-full focus:ring-2 focus:ring-orange-500 focus:border-transparent text-sm" }), _jsx("button", { onClick: handleSendMessage, disabled: !message.trim(), className: "bg-gradient-to-r from-orange-400 to-red-500 text-white p-2 rounded-full hover:from-orange-500 hover:to-red-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed", children: _jsx(Send, { className: "h-4 w-4" }) })] }) })] }))] }));
};
