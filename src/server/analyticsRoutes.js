...
router.get('/api/analytics/course-engagement', async (_, res) => {
    try {
        const result = await pool.query(`SELECT course_id, AVG(avg_progress) AS avg_progress, COUNT(active_users) AS active_users
