...
export const useRealtimeSync = (options = {}) => {
    const { userId, channels = ['course_assignments', 'user_progress', 'enrollments'], onEvent, onError, enabled = true } = options;
    const subscriptionsRef = useRef([]);
    const supabaseRef = useRef(null);
    const reconnectTimeoutRef = useRef();
    const reconnectAttemptsRef = useRef(0);
    const handleRealtimeEvent = useCallback((payload, eventType, channel) => {
        const event = {
            type: eventType,
            payload,
            timestamp: Date.now(),
    ...
    const broadcastUpdate = useCallback(async (eventType, data) => {
