...
export const useCourseProgress = (courseId) => {
    ...
    const enrollInCourse = async () => {
        try {
            if (!hasSupabaseConfig)
                throw new Error('User not authenticated (Supabase disabled)');
            const supabase = await getSupabase();
            if (!supabase)
    ...
    const updateCourseProgress = async () => {
        ...
        `)
            ...
            const completedLessons = courseLessons.filter((lesson) => lessonProgress[lesson.id]?.completed).length;
    ...
    const saveReflection = async (lessonId, content) => {
        try {
            if (!hasSupabaseConfig)
                throw new Error('User not authenticated (Supabase disabled)');
            const supabase = await getSupabase();
    ...
    const submitQuizAttempt = async (lessonId, answers, score, maxScore) => {
        try {
            if (!hasSupabaseConfig)
                throw new Error('User not authenticated (Supabase disabled)');
            const supabase = await getSupabase();
