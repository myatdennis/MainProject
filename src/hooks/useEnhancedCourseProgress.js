...
export const useEnhancedCourseProgress = (courseId, options = {}) => {
    const { userId = 'demo-user', enableAutoSave = true, enableRealtime = true, autoSaveInterval = 30000 } = options;
    // Core state
    ...
    const updateLessonProgress = useCallback(async (lessonId, moduleId, updates) => {
        try {
    ...
    const markLessonComplete = useCallback(async (lessonId, moduleId, finalQuizScore) => {
    ...
    // Save reflection
    const saveReflection = useCallback(async (lessonId, content) => {
        try {
            // Check if we're in demo mode
            const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
            const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY;
    ...
    const updateLessonProgressFromSync = useCallback((syncData) => {
    ...
    const setActiveLessonTracking = useCallback((lessonId) => {
