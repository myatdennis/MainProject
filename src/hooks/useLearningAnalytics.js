...
export const useLearningAnalytics = (options = {}) => {
    const { userId = 'demo-user', courseId, enabled = true, trackingInterval = 30000, // 30 seconds
    idleThreshold = 300000, // 5 minutes
    autoSubmit = true } = options;
    ...
    const trackActivity = useCallback((type, data, duration) => {
    ...
    const updateMetrics = useCallback((type, data, duration) => {
    ...
    const trackLessonStart = useCallback((lessonId, lessonTitle) => {
        trackActivity('lesson_start', { lessonId, lessonTitle, courseId });
    }, [trackActivity, courseId]);
    const trackLessonComplete = useCallback((lessonId, completionData) => {
        trackActivity('lesson_complete', { lessonId, courseId, ...completionData });
    }, [trackActivity, courseId]);
    const trackQuizAttempt = useCallback((lessonId, quizData) => {
        trackActivity('quiz_attempt', { lessonId, courseId, ...quizData });
    }, [trackActivity, courseId]);
    const trackVideoInteraction = useCallback((action, videoData) => {
        trackActivity(action === 'play' ? 'video_play' : 'video_pause', {
            courseId,
            ...videoData
        }, videoData.duration);
    }, [trackActivity, courseId]);
    const trackReflectionSave = useCallback((lessonId, reflectionData) => {
    ...
    const getAnalyticsSummary = useCallback(() => {
        ...
        return {
            ...
            totalTimeSpent: sessions.reduce((total, session) => total + (session.metrics?.timeSpent || 0), 0),
            averageEngagementScore: sessions.length > 0 ?
                sessions.reduce((total, session) => total + (session.metrics?.engagementScore || 0), 0) / sessions.length : 0,
