...
const apiFetch = async (path, options = {}, params) => {
    const resolvedPath = withQuery(path, params);
    return apiRequest(resolvedPath, options);
};
...
export const listDocuments = async (opts) => {
    const json = await apiFetch('/api/admin/documents');
    ...
    if (docs.length === 0) {
        ...
        const refreshed = await apiFetch('/api/admin/documents');
...
export const recordDownload = async (id) => {
    const json = await apiFetch(`/api/admin/documents/${id}/download`, {
        method: 'POST'
    });
    return mapDocumentRecord(json.data);
};
export const updateDocument = async (id, patch) => {
    const json = await apiFetch(`/api/admin/documents/${id}`, {
        method: 'PUT',
