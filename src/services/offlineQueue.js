...
const readQueueFromStorage = async () => {
    if (typeof indexedDB === 'undefined')
        return [];
    try {
...
const writeQueueToStorage = async (snapshot) => {
    if (typeof indexedDB === 'undefined')
        return;
    try {
        const db = await openQueueDb();
...
const compactItem = (item) => {
    try {
        if (item.type === 'progress-event' && item.action === 'progress_update') {
            ...
            if (p.user_id)
                compactPayload.user_id = p.user_id;
            if (p.lesson_id)
                compactPayload.lesson_id = p.lesson_id;
            if (typeof p.resume_at_s === 'number')
                compactPayload.resume_at_s = p.resume_at_s;
            if (typeof p.percent === 'number')
                compactPayload.percent = p.percent;
            if (p.course_id && !p.lesson_id)
                compactPayload.course_id = p.course_id;
...
const dedupeQueue = () => {
    ...
    const toKeep = new Set([...latestByLesson.values(), ...latestSnapshot.values()].map((i) => i.id));
    const result = [];
    const seen = new Set();
    for (const item of queue) {
...
export const getOfflineQueueSnapshot = () => [...queue];
export const subscribeOfflineQueue = (listener) => {
    listeners.add(listener);
    return () => listeners.delete(listener);
};
export const setQueueFullHandler = (handler) => {
...
const enqueueInternal = async (item) => {
...
export const enqueueOfflineItem = async (item) => {
...
const computeBackoff = (attempts = 0) => {
    const base = 2000;
    const multiplier = Math.pow(2, attempts);
    return Math.min(60000, base * multiplier);
};
export const processOfflineQueue = async (type, processor) => {
    await initializeOfflineQueue();
    if (queue.length === 0) {
...
export const hasPendingItems = (type) => {
    if (type) {
        return queue.some((item) => item.type === type);
    }
    return queue.length > 0;
};
export const enqueueProgressSnapshot = async (snapshot) => {
    return enqueueOfflineItem({
        type: 'progress-snapshot',
        userId: String(snapshot.userId ?? ''),
        courseId: String(snapshot.courseId ?? ''),
        moduleId: snapshot.moduleId,
        lessonId: Array.isArray(snapshot.lessonIds) ? snapshot.lessonIds[0] : undefined,
        action: 'course_snapshot',
        payload: snapshot,
        priority: 'medium',
    });
...
export const removeOfflineItem = async (id) => {
    await initializeOfflineQueue();
