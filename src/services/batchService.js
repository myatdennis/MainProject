...
export function enqueueProgress(input) {
    const evt = {
        ...input,
...
export function enqueueAnalytics(input) {
    const evt = {
        ...input,
        clientEventId: input.clientEventId || uid(),
        timestamp: input.timestamp || now(),
...
export async function flushProgress() {
    ...
    try {
        const res = await apiRequest('/api/client/progress/batch', { method: 'POST', body: JSON.stringify({ events: batch }), timeoutMs: 10000 });
        // On partial failure, requeue failed items with backoff
        const failed = (res.failed || []).map((f) => f.id);
        if (failed.length > 0) {
            const map = new Map(batch.map((e) => [e.clientEventId, e]));
            failed.forEach((id) => {
                const evt = map.get(id);
                if (evt)
                    progressQueue.unshift(evt);
            });
...
export async function flushAnalytics() {
    ...
    try {
        const res = await apiRequest('/api/analytics/events/batch', { method: 'POST', body: JSON.stringify({ events: batch }), timeoutMs: 10000 });
        const failed = (res.failed || []).map((f) => f.id);
        if (failed.length > 0) {
            const map = new Map(batch.map((e) => [e.clientEventId, e]));
            failed.forEach((id) => {
