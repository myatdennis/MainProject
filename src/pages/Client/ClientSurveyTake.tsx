  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Card tone="muted" padding="lg" className="space-y-2">
        <h1 className="font-heading text-2xl font-bold text-charcoal">{survey?.title ?? 'Survey'}</h1>
        {survey?.description && <p className="text-sm text-slate/70">{survey.description}</p>}
      </Card>
      ...
      {error ? (
        ...
        <Card tone="muted" padding="lg" className="space-y-4 text-center">
        ...
        <Card tone="muted" padding="lg" className="space-y-5">
          {questions.length === 0 ? (
            ...
            questions.map((question, index) => {
              ...
              return (
                <div key={question.id} className="space-y-3 rounded-xl border border-slate/20 bg-white p-4">
                  <p className="text-sm font-semibold text-charcoal">
                    {index + 1}. {question.title}
                    {question.required && <span className="ml-1 text-red-500">*</span>}
                  </p>
                  {question.description && <p className="text-xs text-slate/70">{question.description}</p>}
                  ...
                  {(question.type === 'open-ended' || question.type === 'other-specify' || question.type === 'file-upload') && (
                    <textarea
