...
vi.mock('../services/batchService', () => ({
    batchService: {
        enqueueProgress: (...args) => enqueueProgressMock(...args),
...
const mockLoadCourseResult = { course: mockCourse, modules: [], lessons: mockLessons, source: 'local' };
const renderCoursePlayer = () => {
    const qc = new QueryClient();
    return render(_jsx(QueryClientProvider, { client: qc, children: _jsx(MemoryRouter, { initialEntries: [`/lms/course/${mockCourse.slug}/lesson/${mockLessons[0].id}`], children: _jsx(Routes, { children: _jsx(Route, { path: "/lms/course/:courseId/lesson/:lessonId", element: _jsx(CoursePlayer, { namespace: "admin" }) }) }) }) }));
};
