...
export const createLearnerNotificationsController = ({ logger, service }) => ({
  list: async (req, res) => {
    try {
      const result = await service.listNotifications({ req, res });
      if (!result) return;
      if (result.raw) return result.payload === null ? res.sendStatus(result.status) : res.status(result.status).json(result.payload);
      ...
      logger.error('learner_notifications_list_failed', { requestId: req.requestId ?? null, message: error?.message ?? String(error) });
      return sendError(res, 500, 'notifications_fetch_failed', 'Unable to load notifications');
    }
  },
  ...
  markRead: async (req, res) => {
    try {
      ...
      logger.error('learner_notifications_mark_read_failed', {
  ...
  delete: async (req, res) => {
    try {
      ...
      logger.error('learner_notifications_delete_failed', {
        requestId: req.requestId ?? null,
        notificationId: req.params.id,
        message: error?.message ?? String(error),
      });
...
export default createLearnerNotificationsController;
