...
import { createTeamHuddleRouter } from './routes/teamHuddle.js';
...
// isVideoTooLargeError and sendVideoTooLargeResponse moved to utils/videoUpload.js
...
app.use('/api/admin/crm', requireAdminAccess, adminCrmRouter({
  logger,
  loadCrmSummary,
  loadCrmActivity,
}));
...
// Inject required helpers/services for orgsRouter
app.use((req, res, next) => {
  req.app.locals.supabase = supabase;
  req.app.locals.ensureSupabase = ensureSupabase;
  req.app.locals.requireUserContext = requireUserContext;
  req.app.locals.invalidateMembershipCache = invalidateMembershipCache;
  req.app.locals.buildMembershipSelect = typeof buildMembershipSelect !== 'undefined' ? buildMembershipSelect : () => '*';
  req.app.locals.getOrgInvitesOrganizationColumnName = typeof getOrgInvitesOrganizationColumnName !== 'undefined' ? getOrgInvitesOrganizationColumnName : async () => 'organization_id';
  req.app.locals.buildActorFromRequest = typeof buildActorFromRequest !== 'undefined' ? buildActorFromRequest : () => ({});
  req.app.locals.recordActivationEvent = typeof recordActivationEvent !== 'undefined' ? recordActivationEvent : async () => {};
  req.app.locals.markActivationStep = typeof markActivationStep !== 'undefined' ? markActivationStep : async () => {};
  next();
});

// Inject required helpers/services for onboardingRouter
app.use((req, res, next) => {
  req.app.locals.supabase = supabase;
  req.app.locals.ensureSupabase = ensureSupabase;
  req.app.locals.requireUserContext = requireUserContext;
  req.app.locals.buildActorFromRequest = typeof buildActorFromRequest !== 'undefined' ? buildActorFromRequest : () => ({});
  req.app.locals.initializeActivationSteps = typeof initializeActivationSteps !== 'undefined' ? initializeActivationSteps : async () => {};
  req.app.locals.recordActivationEvent = typeof recordActivationEvent !== 'undefined' ? recordActivationEvent : async () => {};
  req.app.locals.createAuditLogEntry = typeof createAuditLogEntry !== 'undefined' ? createAuditLogEntry : async () => {};
  req.app.locals.normalizeOrgRole = typeof normalizeOrgRole !== 'undefined' ? normalizeOrgRole : (r) => r;
  req.app.locals.upsertOrganizationMembership = typeof upsertOrganizationMembership !== 'undefined' ? upsertOrganizationMembership : async () => {};
  req.app.locals.markActivationStep = typeof markActivationStep !== 'undefined' ? markActivationStep : async () => {};
  req.app.locals.createOrgInvite = typeof createOrgInvite !== 'undefined' ? createOrgInvite : async () => ({});
  req.app.locals.fetchOnboardingProgress = typeof fetchOnboardingProgress !== 'undefined' ? fetchOnboardingProgress : async () => ({});
  req.app.locals.firstRow = typeof firstRow !== 'undefined' ? firstRow : (r) => r && r[0];
  req.app.locals.ensureUniqueOrgSlug = typeof ensureUniqueOrgSlug !== 'undefined' ? ensureUniqueOrgSlug : async (s) => s;
  req.app.locals.DEFAULT_ORG_PLAN = typeof DEFAULT_ORG_PLAN !== 'undefined' ? DEFAULT_ORG_PLAN : 'default';
  req.app.locals.DEFAULT_ORG_TIMEZONE = typeof DEFAULT_ORG_TIMEZONE !== 'undefined' ? DEFAULT_ORG_TIMEZONE : 'UTC';
  req.app.locals.INVITE_BULK_LIMIT = typeof INVITE_BULK_LIMIT !== 'undefined' ? INVITE_BULK_LIMIT : 10;
  req.app.locals.normalizeInviteNote = typeof normalizeInviteNote !== 'undefined' ? normalizeInviteNote : (n) => n;
  req.app.locals.requireOrgAccess = typeof requireOrgAccess !== 'undefined' ? requireOrgAccess : async () => true;
  req.app.locals.getOrgInvitesOrganizationColumnName = typeof getOrgInvitesOrganizationColumnName !== 'undefined' ? getOrgInvitesOrganizationColumnName : async () => 'organization_id';
  req.app.locals.fetchOrganizationSummary = typeof fetchOrganizationSummary !== 'undefined' ? fetchOrganizationSummary : async () => ({});
  req.app.locals.deliverInviteEmail = typeof deliverInviteEmail !== 'undefined' ? deliverInviteEmail : async () => ({});
  req.app.locals.normalizeOrgInviteRow = typeof normalizeOrgInviteRow !== 'undefined' ? normalizeOrgInviteRow : (i) => i;
  req.app.locals.logOrganizationsEvent = typeof logOrganizationsEvent !== 'undefined' ? logOrganizationsEvent : () => {};
  next();
});
// All onboarding/invite endpoints
app.use('/api/admin/onboarding', authenticate, onboardingRouter);

// Inject required helpers/services for adminOrgProfilesRouter
app.use((req, res, next) => {
  req.app.locals.supabase = supabase;
  req.app.locals.ensureSupabase = ensureSupabase;
  req.app.locals.requireOrgAccess = typeof requireOrgAccess !== 'undefined' ? requireOrgAccess : async () => true;
  req.app.locals.requireAdminAccess = typeof requireAdminAccess !== 'undefined' ? requireAdminAccess : async () => true;
  req.app.locals.handleOrgProfileBundleRequest = typeof handleOrgProfileBundleRequest !== 'undefined' ? handleOrgProfileBundleRequest : async () => {};
  req.app.locals.handleOrgProfileUpsert = typeof handleOrgProfileUpsert !== 'undefined' ? handleOrgProfileUpsert : async () => {};
  req.app.locals.mapContactResponse = typeof mapContactResponse !== 'undefined' ? mapContactResponse : (r) => r;
  req.app.locals.firstRow = typeof firstRow !== 'undefined' ? firstRow : (r) => r && r[0];
  next();
});
// All org-profiles admin endpoints
app.use('/api/admin/org-profiles', adminOrgProfilesRouter);
...
app.use(
  ...
  createCourseCatalogRouter({
    ...
    isDemoOrTestMode,
    isTestMode,
    defaultSandboxOrgId: DEFAULT_SANDBOX_ORG_ID,
    ensureSupabase,
    requireUserContext,
    pickOrgId,
    ...
    logStructuredError,
    courseModulesWithLessonFields: COURSE_MODULES_WITH_LESSON_FIELDS,
...
// Analytics batch ingestion extracted to `server/routes/analytics.js`
// See: server/routes/analytics.js
...
const resolveDocumentTargetOrg = async (req, res, context, rawPayload = {}, { surface = 'admin.documents.create' } = {}) => {
  ...
  const platformScopedOrgIds = context?.isPlatformAdmin
    ? Array.from(
        new Set(
          [
            ...(Array.isArray(context?.organizationIds) ? context.organizationIds : []),
  ...
  const fallbackCandidates = Array.from(
    new Set([headerOrgId, contextOrgId, ...writableOrgIds, ...platformScopedOrgIds].filter(Boolean)),
  );
...
app.use(
  ...
  createDocumentsRouter({
    ...
    documentsBucket: DOCUMENTS_BUCKET,
    documentUrlTtlSeconds: DOCUMENT_URL_TTL_SECONDS,
    firstRow,
    normalizeLegacyOrgInput,
  }),
);
...
app.use(
  ...
  createAdminSurveysRouter({
    ...
    hdiResponseShapes: HDI_RESPONSE_SHAPES,
    toHdiRecord,
  }),
);
...
app.use(
  ...
  createAdminSurveyAssignmentsRouter({
    ...
    shouldUseAssignmentWriteFallback,
    isDemoOrTestMode,
    surveyAssignmentType: SURVEY_ASSIGNMENT_TYPE,
    detectAssignmentsUserIdUuidColumnAvailability,
    getAssignmentsOrgColumnName,
    fetchOrgMembersWithProfiles,
    coerceOrgIdentifierToUuid,
...
app.use(
  ...
  createClientSurveysRouter({
    ...
    createHdiResponseEnvelope,
    hdiResponseShapes: HDI_RESPONSE_SHAPES,
    hdiMetadataContractVersion: HDI_METADATA_CONTRACT_VERSION,
    firstRow,
  }),
);
...
// Analytics events listing extracted to `server/routes/analytics.js`
// See: server/routes/analytics.js
...
// Analytics single-event ingest extracted to `server/routes/analytics.js`
// See: server/routes/analytics.js

// Analytics journeys upsert extracted to `server/routes/analytics.js`
// See: server/routes/analytics.js
...
// Analytics journeys fetch extracted to `server/routes/analytics.js`
// See: server/routes/analytics.js
