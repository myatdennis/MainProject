-- Final follow-up for Supabase linter warnings:
--   - auth_rls_initplan on public.org_invites policies
--   - duplicate_index on public.user_course_progress
--
...
-- ---------------------------------------------------------------------------
-- org_invites policies: force wrapped auth helper calls
-- ---------------------------------------------------------------------------
...
CREATE POLICY "org_invites_insert"
  ...
  WITH CHECK (
    ...
    AND (
      organization_id = ANY (ARRAY(SELECT organization_id FROM get_user_organization_ids()))
      OR EXISTS (
        SELECT 1
        FROM public.organization_memberships AS m
        WHERE m.organization_id = org_invites.organization_id
...
-- ---------------------------------------------------------------------------
-- user_course_progress duplicate-index cleanup
-- Drop user_course_progress_unique only when identical to pkey.
-- Handles both index-backed unique constraints and plain indexes.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  pkey_oid oid;
