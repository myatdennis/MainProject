...
-- 1) auth_rls_initplan on org_invites + course_media_assets policies
-- 2) duplicate_index on public.user_course_progress
--
...
-- ---------------------------------------------------------------------------
-- org_invites policies: ensure auth helpers are called as (SELECT auth.*)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- course_media_assets policy: ensure auth helper is called as (SELECT auth.uid())
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- duplicate_index: drop user_course_progress_unique only when identical to pkey
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  pkey_oid oid;
  dup_oid oid;
  pkey_is_same boolean := false;
...
BEGIN
  ...
  IF pkey_oid IS NOT NULL AND dup_oid IS NOT NULL THEN
