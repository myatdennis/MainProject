-- Keep the legacy upsert_course_full RPC return contract as text, but align
-- all internal identifiers with the current UUID-backed learning schema.
...
begin
  ...
  if nullif(p_course->>'id', '') is not null then
    v_course_id := trim(p_course->>'id')::uuid;
  else
  ...
  if p_modules is not null then
    for m in select * from jsonb_array_elements(p_modules) loop
      if nullif(m->>'id', '') is not null then
        present_module_ids := array_append(present_module_ids, trim(m->>'id')::uuid);
      end if;
    end loop;
  end if;
      ...
      if (m ? 'lessons') then
        for l in select * from jsonb_array_elements(m->'lessons') loop
          if nullif(l->>'id', '') is not null then
            present_lesson_ids := array_append(present_lesson_ids, trim(l->>'id')::uuid);
          end if;
        end loop;
      end if;
